...
def test_location(web_browser):
    ...
    page.location.click()
    page.location_input.send_keys("Иван")
    page.first_element_in_location.click()
    page.wait_page_loaded(10)

    assert page.location.get_text() == "Иван"
...
def test_express_page(web_browser):
    ...
    assert 'https://www.ozon.ru/category/supermarket-25000/?miniapp=supermarket' in page.get_current_url()
...
def test_ozon_card_page(web_browser):
    ...
    assert 'https://bank.ozon.ru/' in page.get_current_url()
...
def test_catalog_electric_page(web_browser):
    ...
    assert page.h3.get_text() == 'Развивайте бизнес и зарабатывайте вместе с нами'
...
def test_certificates_menu_page(web_browser):
    ...
    assert page.h1.get_text() == 'ozon Электронный подарочный сертификат Миллион подарков (3000)'
...
def test_oplata_page(web_browser):
    ...
    assert 'https://www.ozon.ru/club/article/kak-mozhno-oplatit-zakaz-na-ozon-48329/' in page.get_current_url()
    assert page.h1.get_text() == 'Как можно оплатить заказ на OZON'
...
def test_check_sort_by_price_low_to_high(web_browser):
    ...
    page = MainPage(web_browser)
    page.search_input.send_keys("Смартфоны")
...
def test_check_sort_by_popularity(web_browser):
    ...
    page = MainPage(web_browser)
    page.search_input.send_keys("ноутбук")
    page.search_btn.click()
    page.wait_page_loaded(10)
    page.sort.click()
...
def test_check_sort_by_rating(web_browser):
    ...
    page = MainPage(web_browser)
    page.search_input.send_keys("моноблок")
    page.search_btn.click()
    page.wait_page_loaded(10)
    page.sort.click()
...
def test_check_sort_by_new(web_browser):
    ...
    page = MainPage(web_browser)
    page.search_input.send_keys("стул")
    page.search_btn.click()
    page.wait_page_loaded(10)
    page.sort.click()
...
def test_check_search(web_browser):
    page = MainPage(web_browser)
    page.search_input.send_keys('samsung')
    page.search_btn.click()

    assert page.products_titles.count() > 0

    for title in page.products_titles.get_text():
        assert 'samsung' in title.lower()
